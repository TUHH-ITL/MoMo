...
bool Motor402::handleInit()
{
  ...
  // Mecanum wheels do not require a position reference. The upstream 0.2.x
  // implementation always attempted homing here even when the bus config set
  // `homing: false`, preventing profile-velocity startup on EPOS4 wheels.
  return true;
}
