...
class MecanumEpos4Controller(Node):
    def __init__(self):
        ...
        self.declare_parameter("startup_retry_delay", 1.0)
        self.declare_parameter("startup_settle_delay", 0.0)
        self.declare_parameter("startup_drive_stagger", 0.0)
        ...
        self.startup_settle_delay = float(
            self.get_parameter("startup_settle_delay").value
        )
        ...
        for index in range(1, 5):
            ...
            self.target_clients.append(
                self.create_client(COTargetDouble, prefix + "/target")
            )
            self.trigger_clients.append(
                {
                    "init": self.create_client(Trigger, prefix + "/init"),
                    "velocity_mode": self.create_client(
                        Trigger, prefix + "/velocity_mode"
                    ),
    ...
    def start_drives(self):
        if self.startup_settle_delay > 0:
            self.get_logger().info(
                f"Waiting {self.startup_settle_delay:.1f}s for the CANopen bus to "
                "finish booting before enabling drives."
            )
    ...
    def control_loop(self):
        ...
        stale = (
            self.timeout > 0.0
            and (self.get_clock().now() - self.last_cmd).nanoseconds * 1e-9
            > self.timeout
        )
        desired = [0.0] * 4 if stale else self.targets
        for i in range(4):
            slowing_down = (
                abs(desired[i]) < abs(self.outputs[i])
                or desired[i] * self.outputs[i] < 0.0
            )
            ...
            changed = (
                self.last_sent[i] is None
                or abs(self.outputs[i] - self.last_sent[i]) >= 1e-6
            )
            ...
            if self.pending[i]:
                continue
            service_changed = (
                self.service_targets[i] is None
                or abs(self.outputs[i] - self.service_targets[i]) >= 1e-6
            )
        ...
        try:
            if not future.result().success:
                ...
                self.get_logger().warn(
                    f"Drive {wheel + 1} rejected velocity target",
                    throttle_duration_sec=2.0,
                )
        ...
        for client in self.target_clients:
            if client.service_is_ready():
                ...
                rclpy.spin_until_future_complete(
                    self, client.call_async(request), timeout_sec=0.5
                )
